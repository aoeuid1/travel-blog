...
export async function generateStaticParams() {
  ...
  const tags = [...new Set(posts.flatMap(p => p.tag ? [p.tag] : []))];
  return tags.map((tag) => ({
    tag: tag,
  }));
}
...
export default function TagPage({ params }: { params: { tag: string } }) {
  ...
  return (
    <main className="container mx-auto p-4">
      <h1 className="text-4xl font-bold mb-8 text-center">Posts tagged with "{params.tag}"</h1>
      <div className="mb-8">
        <MapLoader posts={posts} fitBounds />
      </div>
